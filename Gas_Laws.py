...
for i, q in enumerate(questions):
    shuffled_choices, correct_label = rotate_choices(q["choices"], q["correct"])
    correct_labels.append(correct_label)
    correct_texts.append(q["choices"][q["correct"]])

    st.write(q["question"])
    answer = st.radio(
        f"Select an answer for Question {i+1}:",
        list(shuffled_choices.keys()),
        format_func=lambda x, d=shuffled_choices: f"{x}: {d[x]}",
        index=None,
        key=f"q{i}"
    )
